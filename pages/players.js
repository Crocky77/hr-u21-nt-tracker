...
function calcAge(dobStr) {
  ...
  // Hattrick dob je "godine + dani" (ne pravi kalendar), ali za MVP je dovoljno:
  const years = Math.floor(days / 112); // ovo NIJE točno za HT, samo placeholder!
  const remDays = days % 112;

  // Napomena: kasnije ćemo zamijeniti s pravom HT logikom (sezonski tjedni).
...
export default function Players() {
  ...
  async function addPlayer(e) {
    ...
    setNewPlayer({ ht_player_id: "", full_name: "", position: "DEF", dob: "", team_type: "U21", status: "watch", notes: "" });
    fetchPlayers();
  }
    ...
    <main style={{ fontFamily: "Arial, sans-serif", padding: 24, maxWidth: 1100, margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        <div>
          ...
          <h1 style={{ margin: "6px 0 0", color: "#c00" }}>Igrači</h1>
      ...
      {role === "admin" && (
        <div style={{ marginTop: 14, border: "1px solid #e5e7eb", borderRadius: 14, background: "#fff", padding: 14 }}>
          ...
          <form onSubmit={addPlayer} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 10 }}>
            ...
            <select
              value={newPlayer.team_type}
              onChange={(e) => setNewPlayer((p) => ({ ...p, team_type: e.target.value }))}
              style={{ padding: 10, borderRadius: 10, border: "1px solid #e5e7eb" }}
            >
          ...
          <div style={{ marginTop: 10, fontSize: 12, opacity: 0.7 }}>
            Napomena: dob (HT) i U21 status ćemo izračunavati kasnije točnom HT formulom. Za sad je ovo baza podataka i bilješke.
          </div>
        </div>
      )}
      ...
      <div style={{ marginTop: 12, fontSize: 12, opacity: 0.7 }}>
        Sljedeće: prava HT dob (21g+111d), U21 status, i bilješke po skautu (notes tablica).
      </div>
    </main>
  );
}
