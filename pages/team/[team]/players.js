...
export default function TeamPlayersPage() {
  ...
  const openPlayer = (playerId) => {
    if (!playerId) return;
    // Use the team-scoped details route. This prevents a bunch of DB mismatches
    // on the old generic /players/[id] page, and fixes NT/U21 context.
    router.push(`/team/${encodeURIComponent(teamSlug || '')}/players/${playerId}`);
  };
  ...
  const fetchPlayers = async () => {
    ...
    try {
      ...
      // Some RPC joins can return duplicate rows (same player multiple times).
      // We de-duplicate client-side by internal player id.
      const rawList = Array.isArray(data) ? data : [];
      const byId = new Map();
      for (const row of rawList) {
        const pid = row?.id ?? row?.player_id;
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '16px 12px' }}>
        ...
        >
          ...
          <div style={{ marginLeft: 'auto', fontSize: 12 }}>
            <Link href={`/team/${teamSlug}`} style={{ textDecoration: 'none', opacity: 0.75 }}>
              Natrag na module
            </Link>
          </div>
        </div>
